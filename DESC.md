...
Интервал страниц задаётся в `user.txt` (`"pages": [from, to]`, см. выше) и
используется в `Ctx.hs` (`getMessages` и `move`). Номер последней страницы темы
берётся из XF2-пагинатора (`input.js-pageJumpPage[max]`, см.
`Post.exractPageNumber`) и печатается в лог. Полный прогон 1..269 дал 854 поста
(849 от 29715000, 5 от orangepink).
